"""Symbol universe loader — fetch S&P 500, Nasdaq 100, Dow 30 tickers from
Wikipedia, cache locally, fall back to a hard-coded list if offline.

Usage
-----
    from ai_trading.data.universe import load_universe
    syms = load_universe(["sp500", "nasdaq100", "dow30"])   # de-duplicated
    syms = load_universe(["all"])                            # all of the above
...
ALL_ALIASES = ("sp500", "nasdaq100", "dow30")
...
def get_index(alias: str, *, refresh: bool = False) -> list[str]:
    """Return tickers for a single index alias. Tries cache → wiki → fallback."""
    alias = alias.lower()
    if alias not in _WIKI_SOURCES:
        raise ValueError(f"Unknown index alias '{alias}'. Use: {list(_WIKI_SOURCES)}")
    if not refresh:
        cached = _read_cache(alias)
        if cached:
            return cached
    fresh = _fetch_wiki(alias)
    if fresh:
...
def main() -> None:
    ...
    parser.add_argument("aliases", nargs="*", default=["all"],
                        help="Index aliases: sp500 nasdaq100 dow30 all (default: all)")
